...
class SchoolScholarshipDisbursementLine(models.Model):
    """
    Represents one debit line of a scholarship disbursement document.
    ...
    posting to the same ``final_account_id`` (the award's own Expense
    Account for a Cash Benefit) but carrying a different
    ...
    _description = "School Scholarship Disbursement - Line"
    _order = "disbursement_id, sequence, id"
    ...
    disbursement_id = fields.Many2one(
        ...
        required=True,
        ondelete="cascade",
        help="Disbursement document this line belongs to.",
    )
    schedule_id = fields.Many2one(
        ...
        required=True,
        ondelete="restrict",
        help="Award Schedule line this disbursement line realizes.",
    )
    ...
    final_account_id = fields.Many2one(
        ...
        required=True,
        ondelete="restrict",
        help="Accounting destination of this line, copied from the "
        "Benefit line's own Account -- the award's Expense Account "
        "for a Cash Benefit line.",
    )
    account_id = fields.Many2one(
        ...
        # ``required=False``, on ``school_scholarship_deduction_line``:
        ...
        "Final Account -- unlike ``school_scholarship_deduction_line``, "
        "this line never posts to a deferred account, since cash "
        "disbursement deferral is out of this module's scope.",
    )
    ...
    # pattern of ``school_scholarship_deduction_line``.
    move_id = fields.Many2one(
        related="disbursement_id.move_id",
        compute_sudo=True,
    )
    date = fields.Date(
        related="disbursement_id.date",
        compute_sudo=True,
    )
    currency_id = fields.Many2one(
        ...
        # to the sibling override on ``school_scholarship_deduction_line``.
        required=False,
    )
    ...
    company_currency_id = fields.Many2one(
        related="disbursement_id.company_currency_id",
        compute_sudo=True,
    )
    ...
    def _compute_account_id(self):
        """Copy Final Account onto the posting Account.
    ...
    def _get_standard_label(self):
        ...
        """
        self.ensure_one()
        return "%s - %s" % (self.disbursement_id.name, self.name)
