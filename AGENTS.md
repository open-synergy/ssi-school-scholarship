...
**Maintenance rule:** this repository has no modules yet. Every backlog item that adds
IK documentation for a module **must** add that module's heading and line(s) here, in
the same commit as the IK files themselves — this index is never allowed to fall behind
