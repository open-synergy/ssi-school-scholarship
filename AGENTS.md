# AGENTS.md

This file indexes **Instruksi Kerja (IK)** documentation across this repository's
modules, for AI/agent consumption. Each module's IK files live under
`<module>/docs/<model_name>/`, one Markdown file per operational action (Create, Edit,
Delete, Confirm, Approve, ...), each structured as Pre-Condition / Flow /
Post-Condition. See each module's `README.rst` ("Work Instruction" section) for the
human-facing rendered links.

**Maintenance rule:** every backlog item that adds IK documentation for a module **must**
add that module's heading and line(s) here, in the same commit as the IK files
themselves — this index is never allowed to fall behind the modules it documents.

## ssi_school_scholarship_deduction_operating_unit

- `docs/school_scholarship_deduction/01-create.md`
- `docs/school_scholarship_deduction_recognition/01-create.md`
